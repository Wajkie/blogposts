...
Note: the search endpoint returns metadata only -- download counts come from a separate API call per package.
...
export async function getNpmPackages(username: string) {
  ...
  const data = await response.json();
  const packages = data.objects.map((obj: NpmSearchResult) => ({
  ...
  // Downloads require a separate API call -- not included in search results
  const withDownloads = await Promise.all(
    packages.map(async (pkg) => {
      ...
      return { ...pkg, downloads: dl.downloads ?? 0 };
    })
  );
...
I fetch additional data from bundlephobia.com (note: this is an unofficial API -- treat it as best-effort and handle failures gracefully):
...
Clicking a package opens a modal. The README is fetched separately from the full package endpoint since it is not included in search results:

```typescript
async function getPackageReadme(name: string): Promise<string> {
...
<Dialog>
  <DialogContent className="max-w-2xl">
    ...
    <div className="flex gap-2">
      <Button asChild><a href={pkg.npm}>NPM</a></Button>
      <Button asChild variant="outline"><a href={pkg.repository}>GitHub</a></Button>
    </div>
  </DialogContent>
</Dialog>
```
...
- **NPM Downloads API**: Monthly download counts
- **Bundlephobia API**: Size metrics (unofficial -- handle gracefully)
...
A portfolio section that shows real impact with numbers, updates automatically every hour, looks professional, and demonstrates API integration skills.
